...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(80);
        // Título
        $this->Cell(30, 10, 'Categorias y sus productos', 0, 0, 'C');
...
$datos = $factura->getCategorias();
foreach ($datos as $row) {
    
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetX(18);
    $pdf->Cell(60, 6, utf8_decode($row['nombre_tipo_prod']), 1, 0, 'C');
    $pdf->Cell(50, 6, utf8_decode($row['nombre_prod']), 1, 0, 'C');
    $pdf->Cell(30, 6, utf8_decode($row['precio_prod']), 1, 0, 'C');
    $pdf->Cell(30, 6, utf8_decode($row['modelo_prod']), 1, 0, 'C');
    $pdf->Ln(6);
}
