...
class Producto extends Validator{
	...
	public function searchProducto($value){
		$sql = "SELECT p.id_producto,p.nombre_prod,p.descripcion_prod,p.cantidad_prod,p.precio_prod,t.nombre_tipo_prod,prov.nombre_prov , p.modelo_prod ,e.estado FROM producto as p INNER JOIN tipo_producto as t ON p.tipo_prod = t.id_tipo_producto INNER JOIN proveedor as prov ON p.proveedor = prov.id_proveedor INNER JOIN estado_prod as e ON p.id_estado_prod = e.id_estado_prod WHERE  p.id_estado_prod = 1 AND nombre_prod LIKE ? OR descripcion_prod LIKE ? ORDER BY nombre_prod";
	...
	public function readProducto(){
		$sql = "SELECT p.id_producto,p.nombre_prod,p.descripcion_prod,p.cantidad_prod,p.precio_prod,t.nombre_tipo_prod,prov.nombre_prov , p.modelo_prod, p.foto_prod,e.estado FROM producto as p INNER JOIN tipo_producto as t ON p.tipo_prod = t.id_tipo_producto INNER JOIN proveedor as prov ON p.proveedor = prov.id_proveedor INNER JOIN estado_prod as e ON p.id_estado_prod = e.id_estado_prod WHERE id_producto = ? AND p.id_estado_prod = 1";
	...
	public function deleteProducto(){
		$sql = "UPDATE producto SET id_estado_prod = 2 WHERE id_producto = ?";
		$params = array($this->id);
		return Database::executeRow($sql, $params);
	}
	#CARRITO ALV#
	...
	public function GetProdDisponibles(){
		$sql="SELECT id_producto, nombre_prod,cantidad_prod, precio_prod, modelo_prod FROM producto WHERE id_estado_prod = 1";
		$params = array(null);
	    return Database::getRows($sql, $params);
	}
	public function GetProdNoDisponibles(){
		$sql="SELECT id_producto, nombre_prod, descripcion_prod, cantidad_prod, precio_prod, modelo_prod FROM producto WHERE id_estado_prod = 2";
		$params = array(null);
	    return Database::getRows($sql, $params);
	}
}
